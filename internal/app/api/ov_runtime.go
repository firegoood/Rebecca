...
func (s *Server) handleOVRuntime(w http.ResponseWriter, r *http.Request) {
	if r.URL.Path != "/api/inbounds/openvpn/runtime" {
	...
	target, err := OVRuntimeTarget(r)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
	response, err := s.OVRuntime(r, target)
	if err != nil {
		writeError(w, http.StatusBadRequest, err.Error())
		return
	}
...
func OVRuntimeTarget(r *http.Request) (string, error) {
	query := r.URL.Query()
	if raw := strings.TrimSpace(query.Get("node_id")); raw != "" {
		nodeID, err := strconv.ParseInt(raw, 10, 64)
		if err != nil || nodeID <= 0 {
			return "", fmt.Errorf("invalid node_id")
		}
		return xrayconfig.NodeTargetID(nodeID), nil
	}
	target := strings.TrimSpace(query.Get("target"))
	if target == "" {
		return "", nil
	}
	normalized, nodeID, err := xrayconfig.ParseTargetID(target)
	if err != nil {
		return "", err
	}
	if normalized == xrayconfig.MasterTargetID {
		return xrayconfig.MasterTargetID, nil
	}
	if nodeID == nil {
...
func (s *Server) OVRuntime(r *http.Request, target string) (OVRuntimeResponse, error) {
	inbounds, err := s.configRepo.FullInbounds(r.Context())
	if err != nil {
		return OVRuntimeResponse{}, err
	}
	...
	for _, inbound := range inbounds {
		...
		if target != "" && !OVInboundMatchesTarget(inbound, target) {
			continue
		}
		...
		serviceIDs, err := OVServiceIDsForInbound(r, s.db, tag)
		if err != nil {
			return OVRuntimeResponse{}, err
		}
		users, err := OVUsersForServices(r, s.db, serviceIDs, OVStringValue(settings["ipv4_pool_cidr"]))
		if err != nil {
			return OVRuntimeResponse{}, err
		}
		...
		response.Inbounds = append(response.Inbounds, OVRuntimeInbound{
			Tag:        tag,
...
func OVRuntimeSettings(inbound map[string]any) map[string]any {
	...
	transport := strings.ToLower(strings.TrimSpace(firstNonEmptyAPIString(out["transport"], out["proto"])))
	if transport != "tcp" && transport != "udp" {
		transport = "udp"
	}
	out["transport"] = transport
	pool := strings.TrimSpace(firstNonEmptyAPIString(out["ipv4_pool_cidr"], out["ipv4PoolCidr"]))
	if pool == "" {
		pool = "10.66.0.0/16"
	}
	out["ipv4_pool_cidr"] = pool
	out["dns_servers"] = OVStringList(firstNonEmptyAPIAny(out["dns_servers"], out["dnsServers"]))
	delete(out, "dnsServers")
	out["server_certificate"] = firstNonEmptyAPIString(out["server_certificate"], out["serverCertificate"])
	delete(out, "serverCertificate")
	out["server_key"] = firstNonEmptyAPIString(out["server_key"], out["serverKey"])
...
func OVServiceIDsForInbound(r *http.Request, db *sql.DB, tag string) ([]int64, error) {
...
func OVUsersForServices(r *http.Request, db *sql.DB, serviceIDs []int64, pool string) ([]OVRuntimeUser, error) {
	if len(serviceIDs) == 0 {
...
func firstNonEmptyAPIString(values ...any) string {
...
func firstNonEmptyAPIAny(values ...any) any {
	for _, value := range values {
		if value == nil {
			continue
		}
