...
func testAdminServer(t *testing.T) (*Server, *sql.DB) {
	t.Helper()
	path := filepath.Join(t.TempDir(), "api-admin.sqlite3")
...
func TestAdminManagementDisableEnableUsersAndNodeOperations(t *testing.T) {
	...
	if stillDisabledByAdmin != 0 {
		t.Fatalf("expected admin_disabled_at cleared after enable, got %d", stillDisabledByAdmin)
	}
